...
{
    public static DateTimeModel ParseDate(string text, int year = 0)
    {
        ...
        {
            var match = RegExpHelper.Match(text, kvp.Value);
            if (match != null)
            {
    ...
    public static TimeSpan? ParseTime(string text)
    {
        ...
        {
            var match = RegExpHelper.Match(text, pattern);
            if (match != null)
            {
    ...
    public static TimeSpan? ParseTimeFromSeconds(string text)
    {
        if (string.IsNullOrEmpty(text))
        {
            return null;
        ...
        var match = RegExpHelper.MatchDouble(text);
        if (match != null)
        {

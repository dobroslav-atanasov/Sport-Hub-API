...
public class JudgeCrawler : BaseCrawler
{
    public JudgeCrawler(ILogger<BaseCrawler> logger, IConfiguration configuration, IHttpService httpService, ICrawlersService crawlersService, IGroupsService groupsService)
    ...
    public override async Task StartAsync()
    {
        ...
        try
        {
            var httpModel = await this.HttpService.GetAsync(this.Configuration.GetSection(CrawlerConstants.PARIS_2024_JUDGES_URL).Value);
            ...
            foreach (var person in model.Persons)
            {
                ...
                try
                {
                    ...
                    //    await File.WriteAllBytesAsync($"Images/Judges/{person.Code}{person.Image.ImageExtension}", imageBytes);
                    //}
                }
                catch (Exception ex)
                {
        ...
        {
            this.Logger.LogError(ex, $"Failed to process url: {this.Configuration.GetSection(CrawlerConstants.PARIS_2024_JUDGES_URL).Value};");
        }

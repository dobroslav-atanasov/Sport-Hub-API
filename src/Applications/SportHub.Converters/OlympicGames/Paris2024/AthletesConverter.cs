...
public class AthletesConverter : BaseConverter
{
    private readonly OlympicGamesRepository<Person> repository;

    public AthletesConverter(ILogger<BaseConverter> logger, ICrawlersService crawlersService, ILogsService logsService, IGroupsService groupsService, IZipService zipService,
    ...
    protected override async Task ProcessGroupAsync(Group group)
    {
        ...
        foreach (var item in persons.Persons)
        {
            var person = new Person
            {
                ...
                TvName = item.TVName,
                FunctionCode = item.MainFunction?.FunctionCode,
                Category = item.MainFunction?.Category,
                Description = item.MainFunction?.Description
            };

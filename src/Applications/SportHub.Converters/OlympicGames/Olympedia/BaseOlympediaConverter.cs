...
public abstract class BaseOlympediaConverter : BaseConverter
{
    ...
    protected ConverterModel PrepareConverterModel(Group group)
    {
        var model = new ConverterModel
        {
            HtmlDocument = this.CreateHtmlDocument(group.Documents.First()),
        };

        model.Title = model.HtmlDocument.DocumentNode.SelectSingleNode("//h1").InnerText.Trim();

        if (int.TryParse(new Uri(group.Documents.OrderBy(x => x.Order).First().Url).Segments.Last(), out var pageId))
        {
            model.PageId = pageId;
        }
